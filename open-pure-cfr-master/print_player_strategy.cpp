			      ...
			      const int max_round )
{
  ...
  if( p == currentPlayer( ag->game, &state ) ) {
    ...
    for( int bucket = 0; bucket < num_buckets; ++bucket ) {
      ...
      printf( "  Bucket %d (%d,%d)(%d,%d):", bucket, rank1, suit1, rank2, suit2 ); // peter printing
      */ 
      //hand_index_t hidx = bucket;
      //uint8_t cards[7];
      if (bucket < 169) {
        //hand_unindex(&preflop_indexer, 1, hidx, cards);

      /* char s0 = SUIT_TO_CHAR[deck_get_suit(cards[0])]; 
      char s1 = SUIT_TO_CHAR[deck_get_suit(cards[1])]; 

      char r0 = RANK_TO_CHAR[deck_get_rank(cards[0])]; 
      char r1 = RANK_TO_CHAR[deck_get_rank(cards[1])]; */

      //int s0 = deck_get_suit(cards[0]); 
      //int s1 = deck_get_suit(cards[1]);

      //int r0 = deck_get_rank(cards[0]); 
      //int r1 = deck_get_rank(cards[1]); 
	...
	if( ( num_choices < 5 ) || ( action_probs[ a ] > 0.001 ) ) {
      ...
      printf( "\n" );
    } // added for the preflop bucket thing 
