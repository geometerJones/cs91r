					      ...
					      const int round ) const
{
  ...
  return bucket;
  */
  uint8_t card0 = hole_cards[ player ][ 0 ];
  uint8_t card1 = hole_cards[ player ][ 1 ];
  uint8_t card0rank = rankOfCard( card0 );
  uint8_t card1rank = rankOfCard( card1 );
  uint8_t card0suit = suitOfCard( card0 );
  uint8_t card1suit = suitOfCard( card1 );

  uint8_t cards[7];
  cards[0] = deck_make_card(card0suit, card0rank);
  cards[1] = deck_make_card(card1suit, card1rank);
