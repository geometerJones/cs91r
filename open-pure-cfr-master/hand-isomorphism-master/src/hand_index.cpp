...
void count_configurations(uint_fast32_t round, uint_fast32_t configuration[], void * data) {
  uint_fast32_t * counts = (uint_fast32_t * ) data; ++counts[round]; //dakota added cast
}

void tabulate_configurations(uint_fast32_t round, uint_fast32_t configuration[], void * data) {
  hand_indexer_t * indexer = (hand_indexer_t*) data; // dakota added cast 
...
void tabulate_permutations(uint_fast32_t round, uint_fast32_t count[], void * data) {
  ...
  while(low < high) {
    ...
    for(uint_fast32_t i=0; i<SUITS; ++i) {
      uint_fast32_t thisthing  = count[pi[i]];
      uint_fast32_t other = indexer->configuration[round][mid][i];
      if (other > thisthing) {
        compare = -1; break;
      } else if (other < thisthing) {
        compare = 1; break;
      }
    }
...
bool hand_indexer_init(uint_fast32_t rounds, const uint8_t cards_per_round[], hand_indexer_t * indexer) {
  ...
  for(uint_fast32_t i=0; i<rounds; ++i) {
    ...
    indexer->configuration[i]              = (uint_fast32_t (*)[SUITS]) calloc(indexer->configurations[i], SUITS*sizeof(uint_fast32_t));
    indexer->configuration_to_suit_size[i] = (uint_fast32_t (*)[SUITS]) calloc(indexer->configurations[i], SUITS*sizeof(uint_fast32_t));
